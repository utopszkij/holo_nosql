...
// property validators definition
interface PropDef {
	pName : string;
	pType : PropType;
	unique? : boolean;
	...
	maxValue? : number;
	maxLength? : number;
	values? : string[]; // values if pType == enum
	reference? : string; // collection Name if pType == pointer
}

// validator object
interface Validator {
	request : string[]; // arrya if property names
	propDefs : PropDef[];
}
...
interface AccessRights {
	reference : {"cName":"", "id":""};
	read : string[]; 	// array of 'Ggroupname' | 'UuserNsame' | 'guest' | 'creator'
	insert : string[];// array of 'Ggroupname' | 'UuserNsame' | 'guest' | 'creator'
	update : string[];// array of 'Ggroupname' | 'UuserNsame' | 'guest' | 'creator'
	del : string[];// array of 'Ggroupname' | 'UuserNsame' | 'guest' | 'creator'
}
...
interface Doc {
	...
	colName? : string;  // if cName == "collections" then this is the collection name
	creator : string; // creator user name
	modifier : string; // last modifier user name
}

// DHT entry object
interface Entry {
	doc : Doc;
	validator? : Validator; // only if cName == "collections"
	accessRights? : AccessRights;
	indexes? : string[]; // only if cName == "collections"
}
...
class Dcursor {
	...
	*/
	next(userName : string) : DocumentHandler {
		var result : DocumentHandler = new DocumentHandler();
		this.pos++;
...
class DocumentHandler {
 	public doc : Doc = {"id":"", "cName":"", "creator":"", "modifier":""};
 	public key : string  = ''; // document or collection hash code
 	public collection : DocumentHandler;
 	public err : string = ''; // error msg
 	public accessRights? : AccessRights;  // accessRights object
 	...
 	*/
 	public init(doc : object, userName : string) : void {
 		this.doc = <Doc> doc;
	 	this.key = '';
 		this.err = '';
 	...
 	* @return void  -- set this.err
 	*/
 	public readByHash(hash : string, userName : string) : void {
		var w : Entry;
		try {
				var s : any = get(hash);
				if (<any> s instanceof Error ) {
 	...
 	*/
 	public readById(cName : string, id : string, userName : string) : void {
 	...
 	* @return void  and set this.err
 	*/
 	public readByName(colName : string, userName : string) : void {
		this.err = '';
 	...
 	*/
 	protected getDocAccessRights(userName: string) : AccessRights {
 		var result : AccessRights = undefined;
	...
	* @return bool
	*/
 	public checkAccessRight(action : string, userName : string) : boolean {
		if (userName == 'root') {
			return true;
		}
		var accessList : string[] = [];
		var result : boolean = false;
		var w : DocumentHandler;
		...
		if (docAccessRights == undefined) {
			if (this.collection.accessRights != undefined) {
			 ...
			 w.readById(docAccessRights.reference.cName,
			 				docAccessRights.reference.id, userName);
	...
	*/
	public _insert() : void {
		var entry : Entry = {"doc":{"id":"", "cName":"", "creator":"", "modifier":""}};
		var links : object[] = [];
		var tag : string;
		var i : number = 0;
		var key : string;
 		...
 		if (this.doc.cName != '') {
			...
			if (this.collection.indexes != undefined) {
	 		  ...
	 		  tag = this.doc.cName+'_all';
				links.push({ Base: App.DNA.Hash, Link: key, Tag: tag});
	...
	* @return void and set this.err
	*/
 	public insert(userName : string) : void {
	...
	* @return void and set this.err
	*/
 	public del(forUpdate : boolean, userName : string) : void {
 		var oldDocumentHandler : DocumentHandler;
 		var oldKey : string = '';
 		var tag : string;
 		var key : string = '';
 		var links : object[];
		...
		if (this.collection.indexes != undefined) {
				...
				links = [];
	 		  tag = this.doc.cName + '_all';
				links.push({ Base: App.DNA.Hash, Link: oldKey, Tag: tag, Option: HC.LinkAction.Del});
				for (i = 0; i < this.collection.indexes.length; i++) {
				  tag = this.doc.cName + '_' +
				  			this.collection.indexes[i] + '_' +
	...
	* @return void and set this.err
	*/
 	public update(userName : string) : void {
		var old : DocumentHandler;
		var i : number;
		if (this.doc.cName == 'collections') {
	...
	* @return void and set this.err
	*/
	doEmpty(userName : string): void {
		var dc : Dcursor = this.doFind(this.doc.colName, 'all', '', 'root');
		var dh : DocumentHandler = dc.next('root');
		while (dh.err == '') {
	...
	*/
 	public doFind(cName : string, propName : string,
 						propValue: string,
 						userName : string): Dcursor {
 		var result : Dcursor = new Dcursor();
 		var tag : string;
 		var i : number;
		var dh : DocumentHandler;
		dh = new DocumentHandler();
 		...
 		var w : GetLinksResponse[] = getLinks(App.DNA.Hash,
						 				 tag,
  					    		 {"Load": false, StatusMask: HC.Status.Live} );
		if (<any> w instanceof Error ) {
	...
	* @return void and set this.err
	*/
	createIndex(propName: string) : void {
		var links : object[] = [];
		var tag : string = '';
		var dc : Dcursor = this.doFind(this.doc.colName, 'all', '', 'root');
		var dh : DocumentHandler = dc.next('root');
		...
		while (dh.err == '') {
				...
				if (dh.doc[propName] != undefined) {
					...
					key = commit('publicStr', {"Links":links});
				}
				dh = dc.next('root');
		}
		return;
	...
	* @return void and set this.err
	*/
	dropIndex(propName: string, oldDoc: object): void {
		var links : object[] = [];
		var tag : string = '';
		var dc : Dcursor = this.doFind(this.doc.colName, 'all', '', 'root');
		var dh : DocumentHandler = dc.next('root');
...
*/
function _checkUser(user: string) : string {
	var result = 'guest';
	var w = user.split('.');
...
function _propCheck(doc, propDef) {
	...
	if (propDef.pType == PropType.integer) {
		if (pValue === parseInt(pValue, 10)) {
		    result = true;
		} else {
		    result = false;
		}
	} else if (propDef.pType == PropType.number) {
      result = (isNaN(parseFloat(pValue)) == true);
...
function genesis () {
  ...
  if ((<any> dc instanceof Error ) || (dc.hashes.length <= 0)) {
		...
		// create collections
  	   ...
  	   w.validator = {"request":["id", "cName", "creator","modifier", "userName", "pswHash"] ,
  	   					"propDefs":[
  	   ...
  	   w.validator = {"request":["id", "cName", "creator","modifier",
  	   					"userName", "groupName"] ,
  	   					"propDefs":[
...
*		info?
*/
function addDocument(jsonParam : string) : string {
...
*		info?
*/
function delDocument(jsonParam : string) : string {
...
*		info?
*/
function updateDocument(jsonParam : string) : string {
	var result = {"state":"OTHER_ERROR", "doc":{}, "info":""};
	var param = JSON.parse(jsonParam);
	var w = new DocumentHandler();
	var userName = _checkUser(param.user);
...
*		info?
*/
function doFind(jsonParam : string) : string {
...
*		info?
*/
function doNext(jsonParam : string) : string {
	var result = {"state":"OTHER_ERROR", "doc":{},
		"accessRights":{}, "validator":{}, "indexes":[], "hashes":[], "pos":-1, "info":""};
	var param = JSON.parse(jsonParam);
	var hd : DocumentHandler;
	var w : Dcursor = new Dcursor();
	var userName = _checkUser(param.user);
...
*		info?
*/
function first(jsonParam : string): string {
	var result = {"state":"OTHER_ERROR", "doc" :{},
		"accessRights" :{}, "validator":{}, "indexes":[], "hashes":[], "pos":-1, "info":""};
	var param = JSON.parse(jsonParam);
	var dh : DocumentHandler;
...
*		info?
*/
function doEmpty(jsonParam : string) : string {
	var result = {"state":"OTHER_ERROR", "info":""};
	var param = JSON.parse(jsonParam);
	var w = new DocumentHandler();
	var userName = _checkUser(param.user);
